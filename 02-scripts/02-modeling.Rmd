...
dem_clim_mort_abs <- dem_clim |> 
  ...
  group_by(k_type) |> 
  summarise(
    mort_abs_type = median(mort_abs, na.rm = T)
  )
...
# that your model has a bit of overdispersion
...
# change legacy + without climate impact
dem_clim_no_cc <- dem_clim

dem_clim_c2p_no_cc <- dem_clim |> 
    filter(
    k_type == "1_Planted" |
      k_type == "2_Planted"
    ) |> 
...
dem_clim_pred_no <- map(dem_clim_new_no, ~predict(
  object = zigamma_model_1, newdata = .x,
  se.fit = TRUE, type = "conditional"
))

arg2_dat_pred_no <- list(
  dat = dem_clim_new_no, 
  pred = dem_clim_pred_no
  )

dem_clim_new_pred_no <- arg2_dat_pred_no |>
  pmap(add_predictions)
...
gg_dem_clim_all_pred_obs <- dem_clim_all_pred_obs |>
  ...
  scale_fill_manual(values = c("#E1BE6A", "#40B0A6", "#CC6677"),
                    guide = "none") +
  scale_color_manual(values = c("#E1BE6A", "#40B0A6", "#CC6677")) +
...
dem_clim_c_pred_obs <- dem_clim_pred_obs_n |> 
  filter(
    df == "dem_clim_c2p_cc" |
      df == "dem_clim_c2p_no_cc" |
      df == "dem_clim_obs_p" |
      df == "dem_clim_c2n_cc" |
      df == "dem_clim_c2n_no_cc" |
      df == "dem_clim_obs_n"
    )
...
gg_dem_clim_c_pred_obs <- dem_clim_c_pred_obs |>
  ...
  mutate(
    ...
    df = recode_factor(
      ...
      "dem_clim_c2p_no_cc" = "Predicted",
      "dem_clim_c2p_cc" = "Predicted under climate change",
      "dem_clim_obs_n" = "Observed",
      "dem_clim_c2n_no_cc" = "Predicted",
      "dem_clim_c2n_cc" = "Predicted under climate change",
    )
  ) |>
  ggplot(aes(x = k_type, y = mortality_pred_obs * 100,
             fill = df, color = df)) +
...
ggsave(
  plot = gg_dem_clim_c_pred_obs +
    theme(
      legend.position = "bottom",
      plot.tag = element_text(face = "bold",
                              size = 9)
      ),
  here("03-results", "si_figures", "transition_plot.png"),
  width = 8, height = 8,
  unit = "cm",
  dpi = 600
)
...
ggsave(
  ...
  here("03-results", "main_figure", "main_plot2.png"),
  width = 9, height = 8,
  unit = "cm",
  dpi = 600
)
